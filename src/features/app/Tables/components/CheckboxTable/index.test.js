...
it("should select multiple checkboxes when multiple checkboxes are clicked", () => {
  ...
  wrapper
    ...
    .at(0)
    .simulate("change", { target: { checked: true, value: "chk-0" } });

  // Simulate clicking on the second checkbox
  wrapper
    .find('input[type="checkbox"]')
    .at(1)
    .simulate("change", { target: { checked: true, value: "chk-1" } });

  // Assert that both checkboxes are selected
  expect(wrapper.find('input[type="checkbox"]').at(0).prop("checked")).toBe(
    true
  );
  expect(wrapper.find('input[type="checkbox"]').at(1).prop("checked")).toBe(
    true
  );
  ...
  wrapper
    .find('input[type="checkbox"]')
    .at(0)
    .simulate("change", { target: { checked: false, value: "chk-0" } });
