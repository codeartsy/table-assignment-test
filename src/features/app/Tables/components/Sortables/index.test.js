...
it("should handle data with missing keys", () => {
  ...
  expect(wrapper.find("td").length).toBe(
    Object.keys(mockData[0]).length * mockData.length
  );
});
...
it("should handle null and undefined values in data keys", () => {
  ...
  expect(wrapper.find("tbody tr").at(0).find("td").at(0).text()).toBe(
    "*Celcom Axiata (LTE)"
  );
  expect(wrapper.find("tbody tr").at(0).find("td").at(1).text()).toBe(
    "CELCOM / My Celcom / 502 19"
  );
  expect(wrapper.find("tbody tr").at(0).find("td").at(2).text()).toBe("Yes");
  expect(wrapper.find("tbody tr").at(1).find("td").at(0).text()).toBe(
    "*DiGi Telecom (LTE)"
  );
  expect(wrapper.find("tbody tr").at(1).find("td").at(1).text()).toBe(
    "DiGi 1800 / DiGi /  MYMY18"
  );
  expect(wrapper.find("tbody tr").at(1).find("td").at(2).text()).toBe("Yes");
  expect(wrapper.find("tbody tr").at(2).find("td").at(0).text()).toBe(
    "*Maxis (LTE)"
  );
  expect(wrapper.find("tbody tr").at(2).find("td").at(1).text()).toBe(
    "U Mobile / MYS 18 / MY 18"
  );
